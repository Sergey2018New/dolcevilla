...
export default function mapMarkers() {
    ...
    function getDataResorts() {
        ...
        if (document.getElementById("map_resorts").dataset.items) {
            result = JSON.parse(
                document.getElementById("map_resorts").dataset.items
            );
        }
        return result;
    }
    ...
    const getContentCard = (item) => {
        ...
        return (
            `<div class="chalet-small-card">
            <a href="${item.link}" class="chalet-small-card__link">
                <div class="chalet-small-card__image-box">
                    <img class="chalet-small-card__image" src=${
                        item.image
                    } srcset=${item.image2x} width="194" height="128" alt="" />
                </div>
                <div class="chalet-small-card__content">
                    <div class="chalet-small-card__row">
                        ${
                            item.name
                                ? `<div class="chalet-small-card__title">${item.name}</div>`
                                : ""
                        }
                        ${
                            item.price
                                ? `<div class="chalet-small-card__price">${item.price}</div>`
                                : ""
                        }
                        ${
                            item.period
                                ? `<div class="chalet-small-card__period">${item.period}</div>`
                                : ""
                        }
                    </div>
                    ${
                        infoList.length
                            ? `<div class="chalet-small-card__info">${infoList}</div>`
                            : ""
                    }
                    <div class="chalet-small-card__more">
                        <span class="chalet-small-card__more-link">` +
            document.getElementById("map").dataset.text +
            `</span>
                    </div>
                </div>
            </a>
    ...
    const loadMap = (mapId, data, cardContent) => {
        async function initMap() {
            ...
            const map = new Map(document.getElementById(mapId), {
                center: { lat: 45.45861, lng: 9.183222 },
                zoom: 12,
                minZoom: 3,
                maxZoom: 17,
    ...
    if (document.getElementById("map")) {
        ...
        async function initMap() {
            var lat = parseFloat(
                document.getElementById("map_chalet").dataset.lat
            );
            var lng = parseFloat(
                document.getElementById("map_chalet").dataset.lng
            );
            const { Map } = await google.maps.importLibrary("maps");
